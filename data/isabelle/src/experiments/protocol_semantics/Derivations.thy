(* 5th Generation Execution theory: Unpaired with Tickets, inlined thread state *)
theory Derivations
imports
  HOL_ext
  Hints
...
inductive_set
  all_traces :: "'s nts \<Rightarrow> 's list set"
  for P      :: "'s nts"
where
  init: "[fst P] \<in> all_traces P"
| proto: 
  "\<lbrakk> tr@[s] \<in> all_traces P;
     (s, inp, s', out) \<in> snd P
   \<rbrakk> \<Longrightarrow>
     tr@[s,s'] \<in> all_traces P"

locale wf_nts =
  fixes P :: "'s nts"
  assumes distinct: "\<And> tr. tr \<in> all_traces P \<Longrightarrow> distinct tr"


section{* Derivations *}

datatype 's node = Init | State 's | Msg execmsg

types 's derivation = "('s node \<times> 's node) set"

fun analz :: "'s node set \<Rightarrow> execmsg \<Rightarrow> 's derivation"
where
  "analz N (Tup x y) = 
     N \<times> {Msg (Tup x y)} \<union> analz N x \<union> analz N y"
| "analz N (Enc m k) =
     N \<times> {Msg (Enc m k)} \<union> analz {Msg (Enc m k), Msg (inv k)} m"
| "analz N m = 
     N \<times> {Msg m}"

inductive_set
  raw_derivs :: "'s nts \<Rightarrow> 's derivation set"
  for P      :: "'s nts"
where
  init: "{(Init, State (fst P))} \<in> raw_derivs P"
| proto:
  "\<lbrakk> D \<in> raw_derivs P;
     Msg ` Option.set inp \<subseteq> Range D;
     State s \<in> Range D;
     \<forall> s'. (State s, State s') \<notin> D;
     (s, inp, s', out) \<in> snd P
   \<rbrakk> \<Longrightarrow>
     insert (State s, State s') D \<union>
     { (Msg xi, State s') | xi. inp = Some xi } \<union>
     (\<Union> xo \<in> Option.set out. analz {State s'} xo)
     \<in> raw_derivs P"
| hash:
  "\<lbrakk> D \<in> raw_derivs P;
     Msg m \<in> Range D
   \<rbrakk> \<Longrightarrow> 
     insert (Msg m, Msg (Hash m)) D \<in> raw_derivs P"
| tuple:
  "\<lbrakk> D \<in> raw_derivs P;
     Msg x \<in> Range D;
     Msg y \<in> Range D
   \<rbrakk> \<Longrightarrow> 
     {Msg x, Msg y} \<times> {Msg (Tup x y)} \<union> D \<in> raw_derivs P"
| enc:
  "\<lbrakk> D \<in> raw_derivs P;
     Msg m \<in> Range D;
     Msg k \<in> Range D
   \<rbrakk> \<Longrightarrow> 
     {Msg m, Msg k} \<times> {Msg (Enc m k)} \<union> D \<in> raw_derivs P"

definition sound :: "'s derivation \<Rightarrow> bool"
where "sound D \<equiv> 
        acyclic D \<and> 
        (\<forall> n \<in> Domain D. (Init, n) \<in> D\<^sup>*) \<and> 
        (\<exists> s. State s \<in> Range D \<and> (\<forall> s'. (State s, State s') \<notin> D))"

definition derivs :: "'s nts \<Rightarrow> 's derivation set"
where "derivs P \<equiv> { D - D' | D D'. D \<in> raw_derivs P \<and> sound (D - D') }"
...
lemma (in wf_nts) dy_conv_derivs:
  "dy P = { (tr, M) 
          | tr M D. 
              D \<in> derivs P \<and> 
              (\<forall> s s'. listOrd tr s s' = ((State s, State s') \<in> D)) \<and>
              Msg ` M = Range D
          }"
  oops
...
lemma D0_in_derivs: "{(Init, State (fst P))} \<in> derivs P"
  apply(simp add: derivs_def sound_def)
  apply(rule exI)+
  apply(auto intro!: raw_derivs.init)+
  done

lemma in_derivsE [elim!]:
  "\<lbrakk> x \<in> derivs P;
     \<And> D D'. \<lbrakk> D \<in> raw_derivs P; x = D - D'; sound (D - D') \<rbrakk> \<Longrightarrow> Q
   \<rbrakk> \<Longrightarrow> Q"
  by(auto simp: derivs_def)

lemma knows_cases: "\<lbrakk> D \<in> derivs P; Msg m \<in> Range D \<rbrakk> \<Longrightarrow> R"
  apply(clarsimp)
  apply(drule raw_derivs.cases)
  apply(simp_all)
  oops

lemma (in wf_nts) dy_imp_derivs:
  "(tr, M) \<in> dy P \<Longrightarrow>
   \<exists> D \<in> derivs P.
     (\<forall> s. (s \<in> set tr) = (State s \<in> Range D)) \<and> 
     (\<forall> s s'. listOrd tr s s' = ((State s, State s') \<in> D)) \<and> 
     (\<forall> m. (m \<in> M) = (Msg m \<in> Range D))"
proof(induct rule: dy.induct)
  case init thus ?case
    apply(simp)
    apply(rule rev_bexI)
    apply(rule D0_in_derivs)
    apply(auto)
    done
next
...
definition mk_unique :: "'s nts \<Rightarrow> (nat \<times> 's) nts"
where "mk_unique P \<equiv> 
  ( (0, fst P)
  , { ((i,s), inp, (Suc i, s'), out) 
    | i s inp s' out.
      (s, inp, s', out) \<in> snd P
...
lemma distinct_all_traces_mk_unique:
  "\<lbrakk> tr \<in> all_traces Q; Q = mk_unique P \<rbrakk> \<Longrightarrow> distinct tr"
proof(induct rule: all_traces.induct)
