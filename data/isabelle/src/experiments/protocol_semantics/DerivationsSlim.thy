...
fun analz :: "execmsg set \<Rightarrow> 's node set \<Rightarrow> execmsg \<Rightarrow> 's dependencies"
where
  "analz M N (Tup x y) = 
    (if (Tup x y \<in> M) 
        then {} 
...
| proto:
  "\<lbrakk> (tr@[s], D) \<in> derivs P;
   ...
   \<rbrakk> \<Longrightarrow>
     ( tr@[s,s']
     , D \<union> 
       (Msg ` Option.set inp) \<times>  {State (Suc (length tr), s')} \<union>
       UNION (Option.set out) (analz (knows D) {State (Suc (length tr), s')})
     ) \<in> derivs P"
...
lemma in_analz_imp_Msg2: 
 "(x,y) \<in> analz M N m \<Longrightarrow> \<exists> m'. y = Msg m'"
 by (induct m arbitrary: N) (auto split: if_splits)

lemma in_Range_analzE: "
  \<lbrakk> x \<in> Range (analz M N m);
    \<And> m'. x = Msg m' \<Longrightarrow> P
  \<rbrakk> \<Longrightarrow> P"
  by(auto dest!: in_analz_imp_Msg2)
...
by (induct rule: derivs_induct)
   (auto simp: nth_append dest: in_analz_imp_Msg2)
...
lemma (in -) analz_Msg1_imp_Msg2:
  "\<lbrakk> (Msg m, y) \<in> analz M N m'; N \<noteq> {} \<rbrakk> \<Longrightarrow> 
   m \<in> M \<or> Msg m \<in> N \<or> (\<exists> x. (x, Msg m) \<in> analz M N m')"
proof(induct m' arbitrary: N y)
  case (Tup m1 m2) thus ?case
...
proof(induct rule: derivs_induct)
  case (proto tr s D inp s' out) thus ?case
    by (fastsimp dest!: in_knowsD analz_Msg1_imp_Msg2)+
qed (fastsimp dest!: in_knowsD)+
...
lemma (in -) finite_analz [intro!]:
  "finite N \<Longrightarrow> finite (analz M N m)"
...
lemma knows_analz: "\<lbrakk> m \<notin> M; N \<noteq> {} \<rbrakk> \<Longrightarrow> m \<in> knows (analz M N m)"
  by (induct m arbitrary: N) (auto simp: knows_iff)

lemma analz_antimono: "M \<subseteq> M' \<Longrightarrow> analz M' N m \<subseteq> analz M N m"
  apply(safe)
  apply(induct m arbitrary: N)
  apply(auto split: if_splits)
  oops (* not true as check for inverse keys runs on the same set *)

lemma (in derivation) knows_analz_closed:
  "\<lbrakk> m \<in> knows D; x \<in> knows (analz (knows D) N m) \<rbrakk> \<Longrightarrow> x \<in> knows D"
proof(induct rule: derivs_induct)
  case init thus ?case by simp
next
  case (hash tr D m') thus ?case
    apply(auto)
    oops (* not true as new keys could be added *)

lemma (in derivation) knows_pairParts_closed:
  "\<lbrakk> m \<in> knows D; x \<in> pairParts m \<rbrakk> \<Longrightarrow> x \<in> knows D"
proof(induct rule: derivs_induct)
  case tuple thus ?case sorry
next
...
next
  ...
  from ext have "?extension D D' x"
    ...
    apply(rule conjI)
    apply(rule ext_deriv.knows_pairParts_closed)
    apply(force)
    apply(simp)
    apply(erule conjE)+
  ...
  (* DEEP Problems with the current explicit model:

       1. Analyzing a message further with keys learned later is not
          possible \<Rightarrow> attacks are missed.

       2. Adding a separate analyze step helps but destroys the property
          that the inversion rules of the inductive set correspond directly
          to the decryption chain rule.

       3. The approach with late checking of soundness requires us to build
          a labeled graph where messages can be learned multiple times.
          Otherwise, we cannot decide later how to learn a message concretely.

     \<Longrightarrow> A small step model as we had it is probably better
  *)
