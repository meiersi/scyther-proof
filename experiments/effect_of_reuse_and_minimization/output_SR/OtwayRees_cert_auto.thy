...
next
  ...
  thus ?thesis by (fastsimp dest: S_ltk_R_S_sec intro: event_predOrdI)
qed (insert facts, ((clarsimp, order?))+)?
...
proof -
  ...
  next
    ...
    next
      case I_1_enc note facts = facts this[simplified]
      thus ?thesis proof(sources! "
                       Enc {| LC ''TT2'', LN ''ni'' tid1, s(|NV ''nr'' tid2|),
                              s(|AV ''I'' tid1|), s(|AV ''R'' tid1|)
                           |}
                           ( K ( s(|AV ''R'' tid1|) ) ( s(|AV ''S'' tid1|) ) ) ")
        case fake note facts = facts this[simplified]
  ...
  next
    ...
    next
      ...
      next
        ...
        have f1: "roleMap r tid5 = Some I" using facts by (auto intro: event_predOrdI)
        have f2: "LN ''ni'' tid5 : knows t" using facts by (auto intro: event_predOrdI)
        note facts = facts ni_first_send[OF f1 f2, simplified]
        thus ?thesis by force
      qed (insert facts, ((clarsimp, order?))+)?
    qed (insert facts, ((clarsimp, order?))+)?
