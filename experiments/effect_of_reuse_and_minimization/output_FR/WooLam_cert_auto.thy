...
next
  ...
  thus ?thesis by (fastsimp dest: S_ltk_A_S_sec intro: event_predOrdI)
next
...
next
  ...
  thus ?thesis by (fastsimp dest: S_ltk_B_S_sec intro: event_predOrdI)
qed (insert facts, ((clarsimp, order?))+)?
...
proof -
  ...
  thus ?thesis proof(sources! "
    ...
    thus ?thesis by (fastsimp dest: A_ltk_A_S_sec intro: event_predOrdI)
  next
    case (S_5_enc tid1) note facts = facts this[simplified]
    thus ?thesis by (fastsimp dest: S_sec_kab intro: event_predOrdI)
  qed (insert facts, ((clarsimp, order?))+)?
qed
...
proof -
  ...
  thus ?thesis proof(sources! "
                   Enc {| LC ''enc52'', s(|AV ''A'' tid0|), s(|NV ''na'' tid0|),
                       ...
                       ( K ( s(|AV ''B'' tid0|) ) ( s(|AV ''S'' tid0|) ) ) ")
    case fake note facts = facts this[simplified]
    thus ?thesis by (fastsimp dest: B_ltk_B_S_sec intro: event_predOrdI)
  next
    case (S_5_enc tid1) note facts = facts this[simplified]
    thus ?thesis by (fastsimp dest: S_sec_kab intro: event_predOrdI)
  qed (insert facts, ((clarsimp, order?))+)?
  ...
  next
    ...
    next
      ...
      next
        ...
        have f1: "roleMap r tid4 = Some B" using facts by (auto intro: event_predOrdI)
        have f2: "LN ''nb'' tid4 : knows t" using facts by (auto intro: event_predOrdI)
  ...
  next
    ...
    next
      ...
      thus ?thesis proof(sources! "
        ...
        thus ?thesis by (fastsimp dest: S_sec_kab intro: event_predOrdI)
      next
        case (A_7_enc tid5) note facts = facts this[simplified]
        thus ?thesis proof(sources! "
        ...
        next
          ...
          thus ?thesis proof(sources! "
                           Enc {| LC ''enc51'', s(|AV ''B'' tid5|), LN ''na'' tid5,
                                  LN ''nb'' tid2, LN ''kab'' tid3
                               |}
          ...
          next
            case S_5_enc note facts = facts this[simplified]
            thus ?thesis proof(sources! "
                             Enc {| LC ''enc6'', LN ''na'' tid4, LN ''nb'' tid2 |}
                                 ( LN ''kab'' tid3 ) ")
              case fake note facts = facts this[simplified]
              thus ?thesis by (fastsimp dest: S_sec_kab intro: event_predOrdI)
            next
