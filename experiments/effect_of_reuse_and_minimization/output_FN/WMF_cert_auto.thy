...
proof -
  ...
  next
    ...
    thus ?thesis proof(sources! " LN ''kIR'' tid1 ")
      ...
      qed (insert facts, ((clarsimp, order?))+)?
    next
      case (S_2_kIR tid2) note facts = facts this[simplified]
      thus ?thesis proof(sources! "
                       Enc {| LC ''step1'', s(|NV ''TimeI'' tid2|), s(|AV ''R'' tid2|),
                              LN ''kIR'' tid1
                           |}
  ...
  next
    ...
    next
      ...
      thus ?thesis proof(sources! " LN ''kIR'' tid2 ")
        ...
        qed (insert facts, ((clarsimp, order?))+)?
      next
        case (S_2_kIR tid3) note facts = facts this[simplified]
        thus ?thesis proof(sources! "
                         Enc {| LC ''step1'', s(|NV ''TimeI'' tid3|), s(|AV ''R'' tid3|),
                                LN ''kIR'' tid2
                             |}
