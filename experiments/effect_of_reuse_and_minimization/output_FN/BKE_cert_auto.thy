...
proof -
  ...
  thus ?thesis proof(sources! "
    ...
    thus ?thesis proof(sources! "
      ...
      thus ?thesis proof(sources! " LN ''kir'' tid0 ")
        case R_2_kir note facts = facts this[simplified]
        thus ?thesis proof(sources! " Hash ( LN ''nr'' tid0 ) ")
          case fake note facts = facts this[simplified]
          thus ?thesis proof(sources! " LN ''nr'' tid0 ")
            case R_2_nr note facts = facts this[simplified]
            thus ?thesis proof(sources! " SK ( s(|AV ''I'' tid0|) ) ")
          ...
          qed
        next
          case (I_3_hash tid2) note facts = facts this[simplified]
          thus ?thesis proof(sources! " SK ( s(|AV ''I'' tid0|) ) ")
          qed (insert facts, ((clarsimp, order?))+)?
        next
          case (R_2_hash tid2) note facts = facts this[simplified]
          thus ?thesis proof(sources! "
                           Enc {| LC ''TT1'', LN ''nr'' tid0, s(|AV ''I'' tid2|) |}
                               ( PK ( s(|AV ''R'' tid2|) ) ) ")
            case fake note facts = facts this[simplified]
            thus ?thesis proof(sources! " LN ''nr'' tid0 ")
              case R_2_nr note facts = facts this[simplified]
    ...
    thus ?thesis proof(sources! "
      ...
      thus ?thesis proof(sources! " LN ''kir'' tid0 ")
        ...
        thus ?thesis proof(sources! " Hash ( LN ''nr'' tid0 ) ")
          ...
          qed
        next
          case (I_3_hash tid3) note facts = facts this[simplified]
          thus ?thesis proof(sources! " SK ( s(|AV ''I'' tid0|) ) ")
          qed (insert facts, ((clarsimp, order?))+)?
        next
          case (R_2_hash tid3) note facts = facts this[simplified]
          thus ?thesis proof(sources! "
                           Enc {| LC ''TT1'', LN ''nr'' tid0, s(|AV ''I'' tid3|) |}
                               ( PK ( s(|AV ''R'' tid3|) ) ) ")
            case fake note facts = facts this[simplified]
            thus ?thesis proof(sources! " LN ''nr'' tid0 ")
              case R_2_nr note facts = facts this[simplified]
...
proof -
  ...
  thus ?thesis proof(sources! "
                   Enc {| LC ''TT2'', Hash ( LN ''ni'' tid1 ), s(|NV ''nr'' tid1|),
                       ...
                       ( PK ( s(|AV ''I'' tid1|) ) ) ")
    case fake note facts = facts this[simplified]
    thus ?thesis proof(sources! " Hash ( LN ''ni'' tid1 ) ")
      case fake note facts = facts this[simplified]
      thus ?thesis proof(sources! " LN ''ni'' tid1 ")
        case I_1_ni note facts = facts this[simplified]
    ...
    next
      case (I_3_hash tid2) note facts = facts this[simplified]
      thus ?thesis proof(sources! "
                       Enc {| LC ''TT2'', Hash ( LN ''ni'' tid2 ), LN ''ni'' tid1,
                              s(|AV ''R'' tid2|), s(|NV ''kir'' tid2|)
                           |}
                           ( PK ( s(|AV ''I'' tid2|) ) ) ")
        case fake note facts = facts this[simplified]
        thus ?thesis proof(sources! " LN ''ni'' tid1 ")
    ...
    next
      case (R_2_hash tid2) note facts = facts this[simplified]
      thus ?thesis proof(sources! "
                       Enc {| LC ''TT1'', LN ''ni'' tid1, s(|AV ''I'' tid2|) |}
                           ( PK ( s(|AV ''R'' tid2|) ) ) ")
        case fake note facts = facts this[simplified]
        thus ?thesis proof(sources! " LN ''ni'' tid1 ")
          case I_1_ni note facts = facts this[simplified]
...
proof -
  ...
  thus ?thesis proof(sources! "
    ...
    thus ?thesis proof(sources! "
      ...
      thus ?thesis proof(sources! " LN ''kir'' tid1 ")
        case R_2_kir note facts = facts this[simplified]
        thus ?thesis proof(sources! " Hash ( LN ''nr'' tid1 ) ")
          case fake note facts = facts this[simplified]
          thus ?thesis proof(sources! " LN ''nr'' tid1 ")
            case R_2_nr note facts = facts this[simplified]
            thus ?thesis proof(sources! " SK ( s(|AV ''I'' tid1|) ) ")
        ...
        next
          case (R_2_hash tid3) note facts = facts this[simplified]
          thus ?thesis proof(sources! "
                           Enc {| LC ''TT1'', LN ''nr'' tid1, s(|AV ''I'' tid3|) |}
                               ( PK ( s(|AV ''R'' tid3|) ) ) ")
            case fake note facts = facts this[simplified]
            thus ?thesis proof(sources! " LN ''nr'' tid1 ")
              case R_2_nr note facts = facts this[simplified]
    ...
    thus ?thesis proof(sources! "
      ...
      thus ?thesis proof(sources! " LN ''kir'' tid1 ")
        ...
        thus ?thesis proof(sources! " Hash ( LN ''nr'' tid1 ) ")
          ...
          qed
        next
          case (I_3_hash tid4) note facts = facts this[simplified]
          thus ?thesis proof(sources! " SK ( s(|AV ''I'' tid1|) ) ")
          qed (insert facts, ((clarsimp, order?))+)?
        next
          case (R_2_hash tid4) note facts = facts this[simplified]
          thus ?thesis proof(sources! "
                           Enc {| LC ''TT1'', LN ''nr'' tid1, s(|AV ''I'' tid4|) |}
                               ( PK ( s(|AV ''R'' tid4|) ) ) ")
            case fake note facts = facts this[simplified]
            thus ?thesis proof(sources! " LN ''nr'' tid1 ")
              case R_2_nr note facts = facts this[simplified]
